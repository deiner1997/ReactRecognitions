...
class create extends Component {
  ...
  componentDidMount() {
    ...
    api.subcategory.getSubcategories()
      ...
      api.recognition.getRecognize(this.props.match.params.id)
      .then(data => {
          this.setState({
            description: data.recognize.description, userAssignId: data.recognize.userAssignId._id, usercreateId: data.recognize.usercreateId._id, 
            categoryId: !isEmpty(data.recognize.categoryId) ? data.recognize.categoryId._id :"", 
            subcategoryId: !isEmpty(data.recognize.subcategoryId) ? data.recognize.subcategoryId._id :"",
            valueId: !isEmpty(data.recognize.valueId) ? data.recognize.valueId._id :"",
            periodId: data.recognize.periodId._id,
            client:data.recognize.client, sherpasNum: data.recognize.sherpasNum, sherpascommittee:  data.recognize.sherpascommittee
          })
      })
      .catch(err => {
  ...
  handleSubmit = event => {
    ...
    if (Object.keys(error).length === 0) {
      api.recognition.updateInfo(this.props.match.params.id,this.state.description, this.state.userAssignId, this.state.usercreateId, this.state.categoryId, this.state.subcategoryId,
        this.state.valueId, this.state.periodId,this.state.client, this.state.sherpasNum, this.state.sherpascommittee)
  ...
  render() {
    ...
    const { error, errorRequest, description } = this.state;
    return (
