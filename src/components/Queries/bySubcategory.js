...
class byCategory extends React.Component {
    ...
    componentDidMount() {
        api.subcategory.getSubcategories()
            .then(data => {
                this.setState({ listSubcategory: data.subcategories })
            })
            .catch(err => {
    ...
    handleChange = event => {
        ...
        api.infoRecognition.searchBySubcategory(event.target.value)
            .then(data => {
                this.setState({ listRecognition: data.recognitions , total: data.recognitions.length,
                    message:  !isEmpty(data.recognitions) ? "" : "In this Subcategory no recognitions have been created"
                });
            })
            .catch(err => {
                this.setState({ errorRequest: err.response.data.message })
    ...
    render() {
        ...
        const { listRecognition, listSubcategory, subcategoryId,total } = this.state;
        return (
            <Fragment >
                <TextField
                    ...
                    helperText="Please select the Subcategory"
                    margin="normal"
                >
                ...
                <Table className={classes.table}>
                    <TableHead>
                        <TableRow>
                            ...
                            <CustomTableCell align="center">Subcategory</CustomTableCell>
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        {listRecognition.map(row => (
                            <TableRow className={classes.row} key={row._id}>
                                ...
                                <CustomTableCell align="center">{!isEmpty(row.subcategoryId)
                                    ? row.subcategoryId.title : null}</CustomTableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
